...
import { addPlannedExpense } from '@/actions/financeActions';
...
export default function AddExpenseModal({ isOpen, onClose, dict, month, onSaved }: AddExpenseModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden anim-up">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-lg font-bold text-gray-900">{dict.modalAddExpenseTitle}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="space-y-1.5">
            ...
            <input required name="name" type="text" className="w-full h-10 px-3 rounded-lg border border-gray-200 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20" placeholder="e.g. Rent, Internet" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input required name="amount" type="number" min="0" step="0.01" className="w-full h-10 pl-8 pr-3 rounded-lg border border-gray-200 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20" placeholder="0.00" />
              </div>
            </div>

            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700">{dict.lblExpectedDate}</label>
              <input required name="expectedDate" type="date" className="w-full h-10 px-3 rounded-lg border border-gray-200 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20" />
            </div>
          </div>

          <div className="space-y-1.5">
            <label className="text-sm font-medium text-gray-700">{dict.lblCategory}</label>
            <select required name="category" className="w-full h-10 px-3 rounded-lg border border-gray-200 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 bg-white">
