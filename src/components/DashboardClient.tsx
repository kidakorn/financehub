...
const LoadingFallback = () => (
  <div className="w-full h-[400px] flex flex-col items-center justify-center gap-6 anim-fade">
    <div className="relative flex items-center justify-center">
      {/* Outer pulsing ring */}
      <div className="absolute w-16 h-16 rounded-full border-2 border-blue-100 animate-ping opacity-75"></div>
      {/* Inner spinning ring */}
      <div className="absolute w-12 h-12 rounded-full border-2 border-transparent border-t-blue-500 border-r-blue-300 animate-spin"></div>
      {/* Center dot */}
      <div className="w-4 h-4 bg-blue-600 rounded-full shadow-sm relative z-10"></div>
    </div>
...
const DashboardOverview = dynamic(() => import('@/components/DashboardOverview'), { loading: () => <LoadingFallback /> });
const PaymentsTab = dynamic(() => import('@/components/PaymentsTab'), { loading: () => <LoadingFallback /> });
const ReportsTab = dynamic(() => import('@/components/ReportsTab'), { loading: () => <LoadingFallback /> });
const FinanceTab = dynamic(() => import('@/components/FinanceTab'), { loading: () => <LoadingFallback /> });
    ...
    <header className="w-full bg-white border-b border-gray-200 sticky top-0 z-30">
      <div className="main-container px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <span className="text-base sm:text-lg font-bold text-gray-900 tracking-tight">{dict.appName}</span>
        </div>

        {/* Level 1: Module Switcher (Center/Desktop) */}
        <div className="hidden md:flex items-center gap-2 bg-gray-100 p-1 rounded-lg">
          <button 
            onClick={() => setActiveModule('fleet')}
            className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors flex items-center gap-2 ${activeModule === 'fleet' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
          >
          ...
          <button 
            ...
            className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors flex items-center gap-2 ${activeModule === 'budget' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
          >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 border-l border-gray-200 pl-3">
            ...
            <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center border border-gray-200 flex-shrink-0 overflow-hidden">
              {userImage ? (
      ...
      <div className="md:hidden flex items-center border-t border-gray-100 divide-x divide-gray-100">
         <button onClick={() => setActiveModule('fleet')} className={`flex-1 py-3 text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${activeModule === 'fleet' ? 'bg-blue-50 text-blue-700' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2"/><circle cx="7" cy="17" r="2"/><path d="M9 17h6"/><circle cx="17" cy="17" r="2"/></svg>
            Fleet Manager
         </button>
         <button onClick={() => setActiveModule('budget')} className={`flex-1 py-3 text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${activeModule === 'budget' ? 'bg-blue-50 text-blue-700' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12V7H5a2 2 0 0 1 0-4h14v4"/><path d="M3 5v14a2 2 0 0 0 2 2h16v-5"/><path d="M18 12a2 2 0 0 0 0 4h4v-4Z"/></svg>
            Personal Budget
         </button>
      </div>

      {/* Level 2: Sub-Nav for Fleet Manager */}
      {activeModule === 'fleet' && (
        <div className="main-container px-4 sm:px-6 flex items-center overflow-x-auto h-12 gap-6 bg-white hide-scrollbar border-t border-gray-100">
          {(['dashboard', 'payments', 'fleet', 'reports'] as FleetTabID[]).map((tab) => {
            const isActive = activeFleetTab === tab;
            let label = '';
...
function AppInner({ userImage }: { userImage?: string | null }) {
  const { state } = useApp();
  ...
  return (
    <div className={`w-full flex flex-col font-sans ${state.lang === 'th' ? 'lang-th' : 'lang-en'} min-h-screen bg-gray-100`}>
      ...
      <main className="main-container w-full px-3 sm:px-6 py-6 sm:py-8 flex-1">
        {activeModule === 'fleet' && (
          <>
            {activeFleetTab === 'dashboard' && <DashboardOverview cars={state.cars} dict={dict} lang={state.lang} />}
            {activeFleetTab === 'payments' && <PaymentsTab cars={state.cars} dict={dict} lang={state.lang} />}
            {activeFleetTab === 'reports' && <ReportsTab cars={state.cars} dict={dict} lang={state.lang} />}
            <div style={{ display: activeFleetTab === 'fleet' ? 'block' : 'none' }}>
              <DashboardList
...
export default function DashboardClient({ initialCars, userName, userImage }: { initialCars: Car[], userName?: string | null, userImage?: string | null }) {
  return (
    <AppProvider initialCars={initialCars} userName={userName}>
      <ToastProvider>
        <AppInner userImage={userImage} />
      </ToastProvider>
    </AppProvider>
  );
}
